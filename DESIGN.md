...
正文使用系统中文无衬线栈，保证 Windows 与移动端可读性。首页拼音姓名保留 Instrument Serif 的电影海报感，后续章节标题使用白色文字叠加阴影，以适配视频背景。

## Layout

- 背景视频为全站通用固定层，首页清晰展示，后续内容通过浅色朦胧玻璃让视频继续透出。
- 入口保留开门体验，主视觉直接使用 `public/assets/generated/celestial-gate-warm.webp`，不再用纯 CSS 拼门。
...
- 项目图片统一提供 WebP 版本，页面引用 WebP，原始 PNG/JPG 保留在仓库中作为备份素材。
- 首屏大门图使用 WebP 并在 `index.html` 中 preload，入口 `<img>` 使用高优先级加载。
- VibeCoding 图片维持用户切换时按需加载，避免一次性请求所有截图。
...
- `public/assets/generated/celestial-gate-warm.webp`
...
- `public/assets/showcase/*.webp`
- `public/assets/resume/avatar.webp`
